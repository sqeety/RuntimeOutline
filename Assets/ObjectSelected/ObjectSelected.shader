...
Shader "Hidden/ObjectSelected"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 _OutlineColor;
            half _OutlineFade;
            sampler2D _ObjectSelectedCopy;

            half4 fragment(Varying i) : SV_Target
            {
                fixed4 copyColor = tex2D(_ObjectSelectedCopy, i.uv);
                ...
                float outLine = saturate(col.b * 10);
                float alpha = 0;
                if (isSelected)
                {
                    alpha = 0;
                ...
                alpha *= _OutlineFade;
                float4 outlineColor = float4(_OutlineColor.rgb * alpha + copyColor.rgb * (1 - alpha), 1.0);
                return outlineColor;
            }
            ENDCG
        }
